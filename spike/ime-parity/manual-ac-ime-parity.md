...
- **入力欄は単一行モデル**（Enter・タブ・折返しは無視。実カーソルと内部追跡列の desync を防ぐため）。
  長文対照は「端末幅に近い長い 1 行」で行う。複数行プロンプト編集は本ハーネスの対象外。
...
| 2 | **長文入力中**（対照群） | `long-input` | 停止 | 端末幅に近い**長い 1 行**の日本語を変換しながら入力（送信しない） | スピナー無しで長文 IME 入力が壊れないか。確定文字列の欠落・重複が無いか |
