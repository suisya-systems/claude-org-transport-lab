...
> **#18 追補（push 一次配送、2026-06-13）— 上表 Issue への加算**（一次設計 = [`broker-native-roles.md`](broker-native-roles.md) §9。すべて broker 枝・加算・renga 不変）:
...
> - **Issue G（dogfood + 既定反転）**: **+K1** = **HARD pre-ratification spike**（Claude Code harness が tool-less な `claude/channel` サーバーを load し idle wake させ renga と coexist するかの実測。不成立なら sidecar 同梱形へ fallback、§9.5）。spawn-flow AC に **3-3b 承認の再導入**を反映。push 経路は Claude Code ≥ v2.1.80 + claude.ai login 前提（pull フォールバックは auth 非依存）。
